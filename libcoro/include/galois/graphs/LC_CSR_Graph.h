...
//    if(prnt) printf("%d ",graph.getEdgeData<typename FED::value_type>(nn));
  }
...
public:
  ...
  void prefetchData(GraphNode N){
      _mm_prefetch(&nodeData[N], _MM_HINT_T0);
  }
  ...
  edge_data_reference
  getEdgeData(edge_iterator ni, MethodFlag GALOIS_UNUSED(mflag) = MethodFlag::UNPROTECTED) {
    // galois::runtime::checkWrite(mflag, false);
    return edgeData[*ni];
  }
