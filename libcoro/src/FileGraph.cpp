 ...
 */
        ...
        void FileGraph::fromFile(const std::string& filename) {
            ...
            uint64 len = W.m -1; // len = n + m + 2
//    printf("len: %d\n",len);

            uint64 numNode_ = atol(W.Strings[0 + 1]) - 1;
            uint64 numEdge_ = atol(W.Strings[1 + 1]);

            int len_64 = 4 + numNode_ + ((numEdge_+1)/2) + ((numEdge_+1)/2); // version | size | numNode | numEdge | outIdx | out | edgeData
            uint64_t* In = new uint64_t [len_64];
            uint32_t* In_32 = (uint32_t*)In;
            In[0] = (uint64_t)1;
            In[1] = (uint64_t)4;
            In[2] = (uint64_t)numNode_;
            In[3] = (uint64_t)numEdge_;

            {parallel_for(uint64 i=3; i < numNode_+3; i++) In[i+1] = (uint64_t)atol(W.Strings[i + 1]);}

            {parallel_for(int i = numNode_+3; i < numNode_+3+numEdge_; i++) In_32[i+5+numNode_] = (uint32_t)atol(W.Strings[i + 1]); }
            W.del();
            std::default_random_engine e;
            std::uniform_int_distribution<uint32_t > u(0,100);
            std::uniform_int_distribution<uint32_t > u2(0,1000);
            std::uniform_int_distribution<uint32_t > u3(0, getBinaryLength(numNode_));
            for(uint64_t i=0; i<numEdge_; i++) {
//                In_32[i+8+numNode_*2+numEdge_+(numEdge_%2)] = (uint32_t)(i%100+1);
//                In_32[i+8+numNode_*2+numEdge_+(numEdge_%2)] = u(e);
                In_32[i+8+numNode_*2+numEdge_+(numEdge_%2)] = u3(e);
            }


            if (len != numEdge_ + numNode_ + 3) {
                std::cout << "Bad input file: length = "<<len<< " n+m+2 = " << numNode_+numEdge_+2 << std::endl;
                exit(-1);
            }

            fromMem((void*)In, 0, 0, buf.st_size);
        }
...
////    printf("len: %d\n",len);
//
//            uint64 n = atol(W.Strings[0 + 1]) - 1;
//            uint64 m = atol(W.Strings[1 + 1]);
////    printf("n %d m %d\n",n,m);
//
//            //uint32* In = (uint32*) malloc((len+m)*sizeof(uint32));
//            uint64_t* In = new uint64_t [len+m-1];//malloc((n+2)*sizeof(uint64_t));
//            //uint64_t* tmp = new (In+n+2) uint64_t [2*m];
//            In[0] = n;
//            In[1] = m;
//
//            {parallel_for(uint64 i=3; i < len; i++) In[i-1] = atol(W.Strings[i + 1]);}
//            W.del();
//            //{parallel_for(uint64 i=0; i < m; i++) tmp[i] = atol(W.Strings[i + 1 + n+2]);}
//            std::default_random_engine e;
//            std::uniform_int_distribution<uint64_t > u(0,100);
//            for(uint64_t i=len; i<len+m; i++) {
////                In[i] = u(e);
//                In[i-1] = (i-len)%100+1;
//            }
//
...
//                exit(-1);
//            }
//            uint64_t* offsets = (uint64_t*)(In+2);
//            uint64_t* edges = (uint64_t*)(In+2+n);
//
//            graphVersion   = 2; //convert_le64toh(*fptr++);
//
//            sizeofEdge = 4;//convert_le64toh(*fptr++);
//            numNodes   = n;//convert_le64toh(*fptr++);
//            numEdges   = m;//convert_le64toh(*fptr++);
//            nodeOffset = 0;
//            edgeOffset = 0;
//            outIdx     = offsets;
//
...
//
//            // skip memory differently depending on file version
//            //uint32_t* edgedata = (uint32_t*)malloc((m)*sizeof(uint32_t));
//            edgeData = (char*)(edges+m);
//        }
